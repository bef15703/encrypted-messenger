...
io.on("connection", (socket) => {
  ...
  socket.on(
    "register_identity",
    async ({ userId, displayName, publicKey }, callback) => {
      ...
      try {
        ...
        console.log(
          `[Registered] "${cleanName}" (${cleanId}) mapped to socket ${socket.id}`,
        );
        ...
        if (pendingPackets.length > 0) {
          console.log(
            `[Mailbox] Delivering ${pendingPackets.length} queued packets to ${cleanId}`,
          );
        ...
        callback({
          success: false,
          error: "Database error during identity registration.",
        });
      }
    },
  );
  ...
  socket.on(
    "update_display_name",
    async ({ userId, displayName }, callback) => {
      const cleanId = userId?.trim().toUpperCase();
      ...
      const activeUserId = userIdsBySocketId.get(socket.id);
      if (!activeUserId || activeUserId !== cleanId) {
      ...
      try {
        ...
        const currentSession = sessionsByUserId.get(cleanId);
        if (currentSession) {
          sessionsByUserId.set(cleanId, {
            ...currentSession,
            displayName: cleanName,
          });
        }
        console.log(`[Profile Updated] "${cleanName}" (${cleanId})`);
        callback({ success: true });
      } catch (err) {
        console.error("[Error] update_display_name database failure:", err);
        callback({
          success: false,
          error: "Failed to update display name in database.",
        });
      }
    },
  );
